% https://github.com/Jonathan-Browning/Kappa-Mu-Shadowed-Fading-Matlab
...
classdef KappaMuShadowed
    ...
    methods(Access = public)
        function obj = KappaMuShadowed(kappa,m,mu,r_hat,NumSamples)
            
            %   Assigning input values
            obj.kappa = input_Check(obj,kappa,'\kappa',0,50);
            obj.m = input_Check(obj,m,'m',0.001,50);
            obj.mu = input_Check(obj,mu,'\mu',0,10);
            obj.r_hat = input_Check(obj,r_hat,'\hat{r}^2',0.5,2.5);
            obj.NumSamples = input_Check(obj,NumSamples,'\NumSamples',1,Inf);
    ...
    methods(Access = private)
        ...
        function data = input_Check(obj, data, name, lower, upper) 
            ...
            % mu must be integer
            if strcmp(name,'\NumSamples') && mod(data, 1) ~= 0
                error(strcat(name,' must be an integer'));
            end
