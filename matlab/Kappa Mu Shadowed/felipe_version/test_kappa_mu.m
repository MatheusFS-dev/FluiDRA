...
% Outros parâmetros
N_events = 100000;  % Número de eventos para a simulação

% Inicializar matriz para armazenar coeficientes de canal + ruído
channel_plus_noise = zeros(N, N_events);
channel = zeros(N, N_events);

%% ---------------------- Simulação de Múltiplos Eventos ---------------------- %%
outage_count = 0;  % Inicializa o contador de "outages"
...
parfor i = 1:N_events
    ...
    % noise = sigma_n * (randn(N, U) + 1i * randn(N, U));  % Ruído complexo
    % 
    ...
    %     outage_count = outage_count + 1;  % Incrementa o contador de "outages"
    % end
end

%% --------------------- Cálculo da Probabilidade de Outage --------------------- %%
outage_probability = outage_count / N_events;  % Probabilidade de Outage
