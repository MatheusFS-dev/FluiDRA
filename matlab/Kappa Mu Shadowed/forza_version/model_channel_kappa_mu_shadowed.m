function [g, correlation] = model_channel_kappa_mu_shadowed(W, N, kappa, m, mu, r_hat)
    ...
    w = r .* exp(1i * theta); % N: número de portas, 1: número de transmissores
    ...
    for k = 1:N
        for l = 1:N
            ...
            correlation(k, l) = (r_hat^2 / 2) * besselj(0, 2 * pi * (k - l) * W / (N - 1));
        end
    end

    % Aplica a correlação ao vetor de canais.
    [U, S] = eig(correlation); % Decomposição em valores singulares para calcular a raiz quadrada da matriz de correlação
    A = U * sqrt(S); % Matriz A que estabelece a correlação entre as portas
    g = A * w; % Aplica a correlação ao vetor de canais
end
